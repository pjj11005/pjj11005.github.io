...
B 트리는 균형 이진 탐색 트리의 일종으로, 자료 구조에서 널리 사용되는 트리이다. 
...
## 3. B 트리의 시간 복잡도 분석
   - B 트리의 삽입, 삭제, 검색 연산의 시간 복잡도는 O(log n) -> 이는 B 트리의 높이에 비례하며, 각 노드에서 키 값을 비교하고 자식 노드로 이동하는 과정을 반복하기 때문
...
2. B* 트리: B 트리의 확장된 형태로, 내부 노드에도 키 값을 저장하는 트리 구조이다. B* 트리는 B 트리보다 더 효율적인 메모리 사용을 제공하며, 트리의 분할과 합병 연산을 더욱 최적화한 형태이다.
