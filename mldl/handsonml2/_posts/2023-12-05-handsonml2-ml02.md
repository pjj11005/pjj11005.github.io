...
- 모델 상용 환경에 배포
    ...
    3. 구글 클라우드 AI 플랬폼과 같은 클라우드에 배포
        1. 모델 저장하고 구글 클라우드 스토리지(GCS)에 업로드 → 구글 클라우드 AI 플랬폼으로 이동하여 새로운 모델 버전 만들고 GCS파일 지정
...
- CAUTION: 데이터의 변화와 트렌드에 의해 훈련된 모델은 정기적으로 재훈련할 필요가 있다.
